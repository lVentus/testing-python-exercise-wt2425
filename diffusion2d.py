...
class SolveDiffusion2D:
    ...
    def initialize_domain(self, w=10., h=10., dx=0.1, dy=0.1):
        assert isinstance(w, float), "Width must be a float"
        assert isinstance(h, float), "Height must be a float"
    ...
    def initialize_physical_parameters(self, d=4., T_cold=300., T_hot=700.):
        assert isinstance(d, float), "Thermal diffusivity must be a float"
